...
SchedulingJobLogsComponent = __decorate([
    ...
    <section id="scheduling-job-logs" class="view" aria-labelledby="sched-title">
              ...
              <tr *ngFor="let j of paginatedJobs">
                ...
                <td class="px-4 py-3">
                  <div class="relative inline-block">
                    <button (click)="toggleExportMenu(j)" [disabled]="!j.results || j.results.length===0" class="px-3 py-1 border rounded bg-white hover:bg-slate-50 inline-flex items-center gap-2">
                      Export Report
                      <svg xmlns="http://www.w3.org/2000/svg" class="w-3 h-3" viewBox="0 0 24 24" fill="none" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M19 9l-7 7-7-7"/></svg>
                    </button>
                    <div *ngIf="exportMenuFor === j.id" class="absolute right-0 mt-2 w-40 bg-white border rounded shadow-sm z-10">
                      <button (click)="exportReport(j,'csv')" class="w-full text-left px-3 py-2 hover:bg-slate-50" [disabled]="!j.results || j.results.length===0">CSV</button>
                      <button (click)="exportReport(j,'excel')" class="w-full text-left px-3 py-2 hover:bg-slate-50" [disabled]="!j.results || j.results.length===0">Excel</button>
                      <button (click)="exportReport(j,'json')" class="w-full text-left px-3 py-2 hover:bg-slate-50" [disabled]="!j.results || j.results.length===0">JSON</button>
                      <button (click)="exportReport(j,'xaml')" class="w-full text-left px-3 py-2 hover:bg-slate-50" [disabled]="!j.results || j.results.length===0">XAML</button>
                    </div>
                  </div>
                </td>
              </tr>
      ...
      <div *ngIf="activeTab==='schedule'">
        <p class="text-sm text-slate-600">Schedule view — placeholder. You can wire a calendar or schedule editor here.</p>
      </div>

      <!-- Output Tab -->
      <div *ngIf="activeTab==='output'">
        <p class="text-sm text-slate-600">Output view — placeholder. Show outputs, artifacts or report history here.</p>
      </div>
